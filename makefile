...
FLAGS_LD    := -Wl,--print-memory-usage,--gc-sections,--relax,--defsym=__rtc_localtime=$(shell powershell [int](Get-Date -UFormat +%s)[0]) -T$(CORE_BASE)/mkl26z64.ld

LIBS        := -larm_cortexM0l_math -lm -lstdc++

DEFINES     := -D__MKL26Z64__ -DTEENSYDUINO=154 -DARDUINO_TEENSYLC -DARDUINO=10807
DEFINES     += -DF_CPU=48000000 -DUSB_MIDI -DLAYOUT_US_ENGLISH
